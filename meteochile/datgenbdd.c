...
int main(int argc, char **argv)
{
  int ix;
  int ind;
  char str_temp[500], str_pre[500], str_hum[500], str_velvi[500], str_dirvien[500], str_ano[500], str_mes[500], str_dia[500], str_hora[500];
  ...
  memset( str_temp, '\0', sizeof(char)*500 );
  ...
  for(ix = 0; (read = getline(&line, &len, fp)) != -1; ix++)
  {
    ...
    //{	    
			 sscanf(pch, "%[^|]|%[^|]|%[^|]|%[^|]|%[^|]|%[^|]|%[^|]|%[^/]/%[^\n]", str_dia, str_mes, str_ano, str_hora, str_temp, str_hum, str_pre, str_dirvien, str_velvi);	                 
				    
			int ind= atoi(str_ano)*8928+atoi(str_mes)*744+atoi(str_dia)*24+atoi(str_hora)-17946047;
			int miltemp= atof(str_temp)*1000;			
